...
import invoicePdfApp from "./invoice-pdf.tsx";
import stripeWebhookApp from "./stripe-webhook.tsx";
import emailRoutes from "./email-routes.tsx";
import adminHubApp from "./admin-hub.tsx";
...
// CRITICAL: These must be mounted BEFORE other sub-apps to avoid auth middleware
console.log('ðŸŒ [Main Server] Mounting PUBLIC routes (no auth)...');
app.route('/', publicRoutesApp);
console.log('âœ… [Main Server] PUBLIC routes mounted successfully');

// ==================== AUTH ROUTES ====================


// Initialize admin user (run once)
app.post("/make-server-c2a25be0/auth/init-admin", async (c) => {
  try {
    const adminEmail = 'admin@duoproservices.ca';
    ...
    if (adminExists) {
      return c.json({ message: 'Admin user already exists', email: adminEmail });
    }
    ...
    return c.json({ error: `Failed to initialize admin: ${error}` }, 500);
  }
});

// List all users (for debugging)
app.get("/make-server-c2a25be0/auth/list-users", async (c) => {
  try {
    ...
    console.error('List users error:', error);
    return c.json({ error: `Failed to list users: ${error}` }, 500);
  }
});

// Reset all users (for debugging)
app.post("/make-server-c2a25be0/auth/reset-all-users", async (c) => {
  try {
    ...
    return c.json({ error: `Failed to reset users: ${error}` }, 500);
  }
});

// Create test user (for debugging)
app.post("/make-server-c2a25be0/auth/create-test-user", async (c) => {
  try {
    ...
    console.error('Create test user error:', error);
    return c.json({ error: `Failed to create test user: ${error}` }, 500);
  }
});

// COMPLETE SYSTEM RESET - Deletes EVERYTHING
app.post("/make-server-c2a25be0/auth/complete-reset", async (c) => {
  try {
    console.log('ðŸ—‘ï¸ Starting complete system reset...');
    ...
    console.log(`âœ… Deleted ${report.authUsersDeleted} auth users`);
    
    // 2. Delete all KV store entries (users, documents, invoices, etc)
    console.log('Step 2: Clearing KV store...');
    const prefixes = ['user:', 'document:', 'invoice:', 'client:', 'onboarding:', 'payment:', 'task:', 'social-post:'];
    for (const prefix of prefixes) {
      const entries = await kv.getByPrefix(prefix);
      for (const entry of entries) {
        await kv.del(entry.key);
        report.kvEntriesDeleted++;
      }
    }
    console.log(`âœ… Deleted ${report.kvEntriesDeleted} KV entries`);
    
    // 3. Delete all files from Storage buckets
    ...
    if (buckets) {
      for (const bucket of buckets) {
        ...
        // Delete the bucket itself if it's a make bucket
        if (bucket.name.startsWith('make-c2a25be0')) {
          await supabase.storage.deleteBucket(bucket.name);
          report.bucketsDeleted++;
        }
      }
    }
    console.log(`âœ… Deleted ${report.storageFilesDeleted} files from ${report.bucketsDeleted} buckets`);
    
    console.log('ðŸŽ‰ Complete system reset finished!');
    
    return c.json({ 
      success: true,
      message: 'ðŸŽ‰ Complete system reset successful! Everything has been deleted.',
      report
    });
  } catch (error) {
    console.error('Complete reset error:', error);
    return c.json({ error: `Failed to complete reset: ${error}` }, 500);
  }
});
...
app.post("/make-server-c2a25be0/personal-info/save", async (c) => {
  console.log('ðŸš€ Personal info save endpoint called');
  ...
  try {
    ...
    try {
      ...
      personalInfo = JSON.parse(rawBody);
      console.log('ðŸ“¦ Personal info parsed successfully');
      console.log('ðŸ“¦ Personal info keys:', Object.keys(personalInfo));
    } catch (parseError) {
      console.error('âŒ JSON parse error:', parseError);
      console.error('âŒ Parse error message:', parseError?.message);
      return c.json({ error: `Invalid JSON in request body: ${parseError.message}` }, 400);
    }

    // For demo purposes - using a default user ID
    // In production, this should use authenticated user
    const demoUserId = 'demo-user-2026';
    console.log('ðŸ‘¤ Using demo user ID:', demoUserId);

    // Save personal info to KV store
    console.log('ðŸ’¾ Saving to KV store...');
    const kvKey = `user:${demoUserId}:personalInfo`;
    ...
    try {
      ...
      console.log(`âœ… KV store save successful`);
    } catch (kvError) {
      console.error('âŒ KV store error:', kvError);
      console.error('âŒ KV error message:', kvError?.message);
      console.error('âŒ KV error stack:', kvError?.stack);
      return c.json({ error: `KV store error: ${kvError.message}` }, 500);
    }

    console.log(`âœ… Personal info saved successfully for ${demoUserId}`);
    ...
    console.error('âŒ Save personal info error:', error);
    ...
    return c.json({ error: `Failed to save personal info: ${error?.message || error}` }, 500);
  }
});

// Test endpoint for personal info (no KV)
app.post("/make-server-c2a25be0/personal-info/test", async (c) => {
  console.log('ðŸ§ª Test endpoint called');
  try {
    const body = await c.req.json();
    console.log('âœ… Test: Body received successfully');
    console.log('âœ… Test: Keys:', Object.keys(body));
    
    return c.json({ 
      success: true,
      message: 'Test successful - request received',
    ...
    return c.json({ error: `Test failed: ${error?.message || error}` }, 500);
  }
});

// Get personal information
app.get("/make-server-c2a25be0/personal-info", async (c) => {
  try {
    // For demo purposes - using a default user ID
    const demoUserId = 'demo-user-2026';

    // Get personal info from KV store
    const personalInfo = await kv.get(`user:${demoUserId}:personalInfo`);
    ...
    return c.json({ error: `Failed to get personal info: ${error}` }, 500);
  }
});
...
app.post("/make-server-c2a25be0/tax-documents/upload", async (c) => {
  try {
    ...
    console.error('âŒ Tax document upload error:', error);
    return c.json({ error: `Upload failed: ${error}` }, 500);
  }
});
...
app.get("/make-server-c2a25be0/tax-documents/list/:year", async (c) => {
  try {
    ...
    console.error('âŒ Tax document list error:', error);
...
app.get("/make-server-c2a25be0/admin/client-documents/:userId", async (c) => {
  try {
    ...
    if (kvError) {
      console.error('âŒ KV error:', kvError);
    ...
    console.error('âŒ Admin client documents error:', error);
...
app.delete("/make-server-c2a25be0/tax-documents/:year/:category/:filename", async (c) => {
  try {
    ...
    if (deleteError) {
      console.error('âŒ Delete error:', deleteError);
    ...
    console.error('âŒ Tax document delete error:', error);
...
// ðŸ”¥ NEW: Get dashboard stats (admin only)
app.get("/make-server-c2a25be0/dashboard/stats", async (c) => {
  try {
    ...
    for (const u of users) {
      // Count all users as clients
      totalClients++;

      // Get tax filings for each user
      const taxFilings = await kv.get(`user:${u.id}:taxFilings`) || [];
      
      if (Array.isArray(taxFilings)) {
        for (const filing of taxFilings) {
          // Count active filings (current year)
          if (filing.year === currentYear && 
              ['in-progress', 'under-review', 'awaiting-payment'].includes(filing.status)) {
            activeFilings++;
          }

          // Count pending invoices
          if (filing.payment && filing.payment.status === 'pending') {
            pendingInvoices++;
          }

          // Calculate monthly revenue (payments made this month)
          if (filing.payment && filing.payment.status === 'paid' && filing.payment.paidAt) {
            const paidDate = new Date(filing.payment.paidAt);
            if (paidDate.getFullYear() === currentYear && 
                paidDate.getMonth() + 1 === currentMonth) {
              monthlyRevenue += filing.payment.amount || 0;
            }
          }
        }
      }
    ...
    const stats = {
      ...
      activeReturns: activeFilings,
      monthlyRevenue,
      clients: {
        clients: [] // Empty for now, can be populated if needed
      },
      financial: {}
    };
    ...
    return c.json({ error: `Failed to get dashboard stats: ${error}` }, 500);
  }
});
...
app.post("/make-server-c2a25be0/tax-filing/reject-report", async (c) => {
  try {
    ...
    console.log(`âŒ [REPORT REJECTED] User ${user.email} rejected ${year} tax return`);
...
app.post("/make-server-c2a25be0/admin/setup-storage-policies", async (c) => {
  try {
    ...
    const manualSQL = `-- ============================================================
-- POLÃTICAS RLS SUPER SIMPLES - GARANTIDO PARA FUNCIONAR
-- ============================================================
  ...
  try {
    ...
    for (const bucketConfig of bucketsToCreate) {
      ...
      if (bucketExists) {
        ...
        if (error) {
          console.error(`âŒ Error creating bucket ${bucketConfig.name}:`, error);
          results.push({
            bucket: bucketConfig.name,
            status: 'error',
...
app.delete("/make-server-c2a25be0/bookkeeping/invoices/:invoiceNumber", async (c) => {
  try {
    ...
    // Find the user and tax filing with this invoice
    const { data: usersData, error: usersError } = await supabase.auth.admin.listUsers();
    ...
    for (const userData of usersData.users) {
      ...
      for (const filing of taxFilings) {
        if (filing.payment?.invoiceNumber === invoiceNumber) {
          foundUser = userData;
          foundFiling = filing;
          break;
        }
    ...
    if (!foundUser || !foundFiling) {
      return c.json({ error: 'Invoice not found' }, 404);
    }

    // Remove the invoice from the tax filing payment
    const metadata = foundUser.user_metadata || {};
    const taxFilings = metadata.taxFilings || [];
    
    const updatedFilings = taxFilings.map((filing: any) => {
      if (filing.payment?.invoiceNumber === invoiceNumber) {
        // Remove invoice-related payment data
        return {
          ...filing,
          payment: {
            ...filing.payment,
    ...
    return c.json({ error: `Failed to delete invoice: ${error}` }, 500);
  }
});
...
app.delete("/make-server-c2a25be0/messages/:clientId/:messageId", async (c) => {
  try {
    ...
    // Delete the message
    await messages.deleteMessage(messageId, clientId);
    ...
    return c.json({ error: `Failed to delete message: ${error}` }, 500);
  }
});
...
// Get value by key
app.get("/make-server-c2a25be0/kv/get", async (c) => {
  try {
...
app.get("/make-server-c2a25be0/kv/getByPrefix", async (c) => {
  try {
    ...
    return c.json({ error: `Failed to get values: ${error}` }, 500);
  }
});

// Set value
app.post("/make-server-c2a25be0/kv/set", async (c) => {
  try {
...
// Delete value
app.delete("/make-server-c2a25be0/kv/del", async (c) => {
  try {
    const key = c.req.query('key');
    ...
    return c.json({ error: `Failed to delete value: ${error}` }, 500);
  }
});
...
app.route('/', roadmapApp);

// Mount fix tax filings routes
app.route('/', fixTaxFilingsApp);

// Mount initial payment routes
app.route('/', initialPaymentApp);

// Mount invoice PDF routes
app.route('/', invoicePdfApp);

// Mount Stripe webhook routes
app.route('/', stripeWebhookApp);

// Mount email routes
app.route('/make-server-c2a25be0/emails', emailRoutes);

// Mount contact email routes
console.log('ðŸ“§ [Main Server] Mounting Contact Email routes...');
app.route('/make-server-c2a25be0', contactEmailApp);
console.log('âœ… [Main Server] Contact Email routes mounted successfully');

// Mount admin hub routes
console.log('ðŸ”§ [Main Server] Mounting Admin Hub routes...');
app.route('/make-server-c2a25be0', adminHubApp);
console.log('âœ… [Main Server] Admin Hub routes mounted successfully');

// Mount admin confirm user routes
console.log('ðŸ”§ [Main Server] Mounting Admin Confirm User routes...');
app.route('/make-server-c2a25be0', adminConfirmUserApp);
console.log('âœ… [Main Server] Admin Confirm User routes mounted successfully');

console.log('ðŸš€ [Main Server] All routes mounted. Server is ready!');
