...
import { getEmailTemplate, shouldSendNotification } from "./emailTemplates.tsx";
import { getCRAAssessmentEmail } from "./craAssessmentEmail.tsx";
...
console.log('🌍 [Main Server] Mounting PUBLIC routes (no auth)...');
app.route('/', publicRoutesApp);
console.log('✅ [Main Server] PUBLIC routes mounted successfully');
...
// Create user with custom email/password (for debugging)
app.post("/make-server-c2a25be0/auth/create-user", async (c) => {
  try {
...
app.get("/make-server-c2a25be0/auth/admin/list-users", async (c) => {
  try {
    const { data, error } = await supabase.auth.admin.listUsers();
...
app.post("/make-server-c2a25be0/auth/admin/reset-user-password", async (c) => {
  try {
...
app.post("/make-server-c2a25be0/auth/complete-reset", async (c) => {
  try {
    console.log('🗑️ Starting complete system reset...');
    ...
    console.log(`✅ Deleted ${report.authUsersDeleted} auth users`);
    ...
    console.log(`✅ Deleted ${report.kvEntriesDeleted} KV entries`);
    ...
    console.log(`✅ Deleted ${report.storageFilesDeleted} files from ${report.bucketsDeleted} buckets`);
    
    console.log('🎉 Complete system reset finished!');
    
    return c.json({ 
      success: true,
      message: '🎉 Complete system reset successful! Everything has been deleted.',
      report
    });
  } catch (error) {
...
// Reset password for existing user (admin endpoint)
app.post("/make-server-c2a25be0/auth/reset-password", async (c) => {
  try {
    const { email, newPassword } = await c.req.json();
    ...
    if (listError) {
      ...
      return c.json({ error: `Failed to find user: ${listError.message}` }, 400);
    }
...
app.get("/make-server-c2a25be0/admin/list-users", async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: `Failed to list users: ${error.message}` }, 400);
    }
...
app.post("/make-server-c2a25be0/admin/delete-user", async (c) => {
  try {
    ...
    try {
      ...
      console.log(`✅ Deleted user profile from KV store`);
    } catch (kvError) {
      console.log(`⚠️ Failed to delete from KV store (might not exist): ${kvError}`);
    }
...
app.post("/make-server-c2a25be0/auth/signin", async (c) => {
  try {
    ...
    if (error) {
      ...
      return c.json({ error: `Sign in failed: ${error.message}` }, 400);
    }
...
app.post("/make-server-c2a25be0/personal-info/save", async (c) => {
  ...
  try {
    ...
    console.log(`✅ Personal info saved successfully for ${demoUserId}`);
...
app.post("/make-server-c2a25be0/personal-info/test", async (c) => {
  ...
  try {
    ...
    console.log('✅ Test: Body received successfully');
    console.log('✅ Test: Keys:', Object.keys(body));
...
console.log('📧 [Main Server] Mounting Contact Email routes...');
app.route('/make-server-c2a25be0', contactEmailApp);
console.log('✅ [Main Server] Contact Email routes mounted successfully');

// Mount admin hub routes
console.log('🔧 [Main Server] Mounting Admin Hub routes...');
app.route('/make-server-c2a25be0', adminHubApp);
console.log('✅ [Main Server] Admin Hub routes mounted successfully');

// Mount admin confirm user routes
console.log('🔧 [Main Server] Mounting Admin Confirm User routes...');
app.route('/make-server-c2a25be0', adminConfirmUserApp);
console.log('✅ [Main Server] Admin Confirm User routes mounted successfully');
