...
function AdminClientDetailPage() {
  ...
  const loadClientData = async () => {
    ...
    try {
      ...
      if (DEMO_MODE) {
        ...
        // Buscar personal_info do KV store
        const { data: personalInfoData, error: personalError } = await supabase
          .from('kv_store_c2a25be0')
          .select('value')
          .eq('key', `personal_info:${userId}`)
          .single();
        ...
        if (!personalInfoData?.value) {
          throw new Error("Client not found - no personal info");
        }

        // Buscar user_metadata
        ...
        const personalInfo = personalInfoData.value;
        ...
        console.log('✅ [AdminClientDetail] Loaded client from KV store');
  ...
  const handleDeleteMessage = async (messageId: string, subject: string) => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_ENDPOINTS.messages(userId || '')}/${messageId}`,
        {
          method: 'DELETE',
          headers: {
  ...
  const loadAllDocuments = async () => {
    ...
    try {
      ...
      if (DEMO_MODE) {
        ...
        // Buscar TODOS os documentos do usuário (todas as chaves que começam com user:userId:documents:)
        ...
        // Organizar documentos por ano
        const documentsByYear: DocumentsByYear[] = [];
        
        if (docsData && docsData.length > 0) {
          // Agrupar por ano
          const yearMap: Record<number, FileData[]> = {};

          for (const doc of docsData) {
            // Extrair ano da chave: user:userId:documents:2025
            const yearMatch = doc.key.match(/documents:(\d{4})/);
            if (!yearMatch) continue;

            const year = parseInt(yearMatch[1]);
            const files = doc.value;
        ...
        console.log(`✅ [AdminClientDetail] Loaded ${documentsByYear.length} years with documents`);
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start gap-2">
                          ...
                          {/* Delete button - only visible on hover for admin messages */}
                          <button
