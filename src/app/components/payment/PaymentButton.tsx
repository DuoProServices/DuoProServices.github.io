  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (errorMsg.includes('demo') || errorMsg.includes('Demo')) {
          toast.info('🎭 Demo Mode: Payment simulation started');
        }
      ...
      if (url.includes('demo-stripe-checkout')) {
        toast.success('🎭 Demo Mode: Opening simulated checkout', {
          description: 'This is a test. No real payment will be processed.',
          duration: 4000,
        });
        
        // In demo mode, simulate success after a short delay
        setTimeout(() => {
          toast.success('✅ Demo payment completed!', {
            description: 'In production, this would redirect to Stripe.',
          });
          if (onSuccess) {
            onSuccess();
          }
