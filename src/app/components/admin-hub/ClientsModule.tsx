...
export default function ClientsModule() {
  ...
  const loadClients = async () => {
    ...
    try {
      ...
      if (DEMO_MODE) {
        console.log('📦 [ClientsModule] Loading REAL clients from Supabase KV store...');
      ...
      console.log('🔄 [ClientsModule] Starting loadClients...');
      ...
      if (!response.ok) {
        ...
        console.error('❌ [ClientsModule] API Error:', errorData);
        throw new Error(errorData.error || "Failed to load clients");
      }

      const data = await response.json();
      console.log('✅ [ClientsModule] Loaded clients:', data.clients?.length || 0);
