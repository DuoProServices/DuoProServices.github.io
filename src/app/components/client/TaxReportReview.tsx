...
export function TaxReportReview({ filing, onApprove, onReject }: TaxReportReviewProps) {
  ...
  const handleDownloadSummaryPDF = async () => {
    // PDF generation removed - tax summary is displayed on screen only
    toast.info(
      language === 'en' ? 'PDF generation has been disabled. You can view the summary on this page.' :
      language === 'fr' ? 'La génération de PDF a été désactivée. Vous pouvez voir le résumé sur cette page.' :
      'A geração de PDF foi desativada. Você pode ver o resumo nesta página.'
    );
  };
